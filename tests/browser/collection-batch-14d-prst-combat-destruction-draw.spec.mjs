...
test("Batch 14D PRST000001 Aonir marks human servants and applies Divine Wrath to damaged undead", async ({page}, testInfo) => {
  ...
  expect(result.text.join(' ')).not.toContain('[Colère');
  expect(result.text.join(' ')).not.toContain('Colère Divine');
...
test("Batch 14D PRST000004 Zarrach replaces a destroyed allied servant with same-cost Orc/Goblin/Troll", async ({page}, testInfo) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    return {play, beforeHand, afterHand:[...player1.hand], added, card, deadCost, events:window.__collectionBatch14D?.events || [], activeFlights:document.querySelectorAll('[data-testid=\"batch14d-hand-flight\"][data-flight-reason=\"zarrach\"]').length};
  });
  expect(result.play.success).toBe(true);
  ...
  expect(result.events.some(event => event.type === "zarrach-servant-death-replacement" && event.success && event.flight)).toBe(true);
  expect(result.events.some(event => event.type === "hand-flight-created" && event.reason === "zarrach" && event.origin === "center")).toBe(true);
  await attachDiagnostics(testInfo, diagnostics);
  expect(diagnostics.pageErrors).toEqual([]);
  expect(blockingConsoleErrors(diagnostics)).toEqual([]);
});
...
test("Batch 14D PRST000006 Éréon draws two cards for the favor owner after opponent draw without recursion", async ({page}, testInfo) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    player1.hand = [];
    player1.drawPile = ["H000001", "H000002", "PRST000006"];
    player2.hand = [];
  ...
  expect(result.after.p1HandIds).not.toContain("PRST000006");
  expect(result.events.filter(event => event.type === "ereon-bonus-draw")).toHaveLength(2);
  expect(result.events.filter(event => event.type === "prst-specific-activated" && event.cardId === "PRST000006")).toHaveLength(1);
  expect(result.events.filter(event => event.type === "hand-flight-created" && event.reason === "ereon" && event.origin === "deck")).toHaveLength(2);
  await attachDiagnostics(testInfo, diagnostics);
  expect(diagnostics.pageErrors).toEqual([]);
  expect(blockingConsoleErrors(diagnostics)).toEqual([]);
});
...
test("Batch 14D PRST000007 Hirin bypasses Rempart and curses enemy servants against their own avatar", async ({page}, testInfo) => {
  ...
  expect(result.enemyBeforeText.join(' ')).not.toContain('  ');
  expect(result.avatarTargetable).toBe(false);
